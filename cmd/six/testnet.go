...
import (
	...
	tmtimetypes "github.com/tendermint/tendermint/types/time"
)
...
Example:
	six testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "six", "Home directory of the node's daemon configuration")
...
func InitTestnet(
	...
	config.Telemetry.PrometheusRetentionTime = 60
	config.Telemetry.EnableHostnameLabel = false
	config.Telemetry.GlobalLabels = [][]string{{"chain_id", chainID}}
	...
	for i := 0; i < numValidators; i++ {
		...
		serverconfig.WriteConfigFile(filepath.Join(nodeDir, "config/app.toml"), config)
	}
