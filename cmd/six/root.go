...
func NewRootCmd() (*cobra.Command, six.EncodingConfig) {
	var (
		config  = six.MakeEncodingConfig()
		context = client.Context{}.
			...
			WithHomeDir(six.DefaultNodeHome)
	)
...
func initRootCmd(rootCmd *cobra.Command, encoding six.EncodingConfig) {
	rootCmd.AddCommand(
		genutilcli.InitCmd(six.ModuleBasics, six.DefaultNodeHome),
		genutilcli.CollectGenTxsCmd(banktypes.GenesisBalancesIterator{}, six.DefaultNodeHome),
		genutilcli.GenTxCmd(six.ModuleBasics, encoding.TxConfig, banktypes.GenesisBalancesIterator{}, six.DefaultNodeHome),
		genutilcli.ValidateGenesisCmd(six.ModuleBasics),
		AddGenesisAccountCmd(six.DefaultNodeHome),
		AddGenesisWasmMsgCmd(six.DefaultNodeHome),
		tmcli.NewCompletionCmd(rootCmd, true),
		testnetCmd(six.ModuleBasics, banktypes.GenesisBalancesIterator{}),
		debug.Cmd(),
	)

	server.AddCommands(rootCmd, six.DefaultNodeHome, appCreatorFunc, appExportFunc, addModuleInitFlags)
	rootCmd.AddCommand(
		rpc.StatusCommand(),
		queryCommand(),
		txCommand(),
		keys.Commands(six.DefaultNodeHome),
	)
}
...
func queryCommand() *cobra.Command {
	...
	six.ModuleBasics.AddQueryCommands(cmd)
...
func txCommand() *cobra.Command {
	...
	six.ModuleBasics.AddTxCommands(cmd)
	cmd.PersistentFlags().String(flags.FlagChainID, "", "The network chain ID")
...
func appCreatorFunc(logger log.Logger, db tmdb.DB, tracer io.Writer, options servertypes.AppOptions) servertypes.Application {
	...
	return six.New(
		...
		six.MakeEncodingConfig(),
		options,
		six.GetWasmEnabledProposals(),
		wasmOpts,
...
func appExportFunc(logger log.Logger, db tmdb.DB, tracer io.Writer, height int64,
	...
	var app *six.App
	if height != -1 {
		app = six.New(logger, db, tracer, false, map[int64]bool{}, homePath, cast.ToUint(options.Get(server.FlagInvCheckPeriod)), config, options, six.GetWasmEnabledProposals(), emptyWasmOpts)

		if err := app.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		app = six.New(logger, db, tracer, true, map[int64]bool{}, homePath, cast.ToUint(options.Get(server.FlagInvCheckPeriod)), config, options, six.GetWasmEnabledProposals(), emptyWasmOpts)
	}
